...
def test_post_add_new_pet_with_valid_data(name='Джек', animal_type='собака', age='7', pet_photo='images/IMG_8522.jpeg'):
...
#
# def test_successful_update_self_pet_info(self, name='Пётр',
#                                          animal_type='Амурский тигр', age=6):
...
def test_successful_update_self_pet_info(name='Пётр', animal_type='Амурский тигр', age=6):
    _, auth_key = pf.get_api_key(valid_email, valid_password)
    _, my_pets = pf.get_list_of_pets(auth_key, "my_pets")
    if len(my_pets['pets']) > 0:
...
def test_post_add_new_pet_without_photo(name='Джек', animal_type='собака', age=7):
    _, auth_key = pf.get_api_key(valid_email, valid_password)
    status, result = pf.post_add_new_pet_without_photo(auth_key, name, animal_type, age)
    assert status == 200
    assert result['name'] == name

def test_post_add_photo_of_pet(pet_photo='images/IMG_8522.jpeg'):
    pet_photo = os.path.join(os.path.dirname(__file__), pet_photo)
    _, auth_key = pf.get_api_key(valid_email, valid_password)
    _, my_pets = pf.get_list_of_pets(auth_key, "my_pets")
    if len(my_pets['pets']) > 0:
...
def test_get_all_pets_with_invalid_filter(filter='not_my_pets'):
    ...
    assert status == 400 or 401 or 402 or 403 or 404 or 405
# 3
def test_post_add_new_pet_with_invalid_name(name='fghfhdhdshfjhajavajfjhajhfajhfhafbjhabfjkbajhbafhjasjahjbjhafsjhbfsjh', animal_type='собака', age='1', pet_photo='images/IMG_8522.jpeg'):
    pet_photo = os.path.join(os.path.dirname(__file__), pet_photo)
    _, auth_key = pf.get_api_key(valid_email, valid_password)
    status, result = pf.post_add_new_pet(auth_key, name, animal_type, age, pet_photo)
    assert status == 400 or 401 or 402 or 403 or 404 or 405
# 4
def test_post_add_new_pet_with_none_name(name='', animal_type='собака', age='1', pet_photo='images/IMG_8522.jpeg'):
    pet_photo = os.path.join(os.path.dirname(__file__), pet_photo)
    _, auth_key = pf.get_api_key(valid_email, valid_password)
    status, result = pf.post_add_new_pet(auth_key, name, animal_type, age, pet_photo)
    assert status == 400 or 401 or 402 or 403 or 404 or 405